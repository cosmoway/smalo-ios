...
public class ZFRippleButton: UIButton {
    ...
    static var rippleColor:UIColor?
    ...
    private func setup() {
        ...
        layer.insertSublayer(rippleBackgroundView.layer, atIndex: 0)
        rippleBackgroundView.layer.insertSublayer(rippleView.layer, atIndex: 0)
    ...
    private func setupRippleView() {
        let size: CGFloat = 400 * CGFloat(ripplePercent)
        let x: CGFloat = (CGRectGetWidth(bounds)/2) - (size/2)
        let y: CGFloat = (CGRectGetHeight(bounds)/2) - (size/2)
        let corner: CGFloat = size/2
        
        rippleView.backgroundColor = ZFRippleButton.rippleColor
